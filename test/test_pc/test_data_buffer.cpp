// Behavioral tests for pmm::DataBuffer<RGB>.
// Verifies the single-slot SPSC contract: producer writes, consumers read
// zero-copy from the same slot via release/acquire atomics.
...
TEST_CASE("DataBuffer: allocate(0) leaves the buffer invalid") {
  ...
  CHECK(b.allocate(0));
  CHECK(!b.valid());
  CHECK(b.try_acquire_read() == nullptr);
}

TEST_CASE("DataBuffer: try_acquire_read returns nullptr before first publish") {
  DataBuffer<RGB> b;
...
TEST_CASE("DataBuffer: publish then read returns the filled slot") {
  ...
  const RGB* r = b.try_acquire_read();
  REQUIRE(r != nullptr);
  CHECK(r[0].r == 1);
  CHECK(r[3].r == 4);
  b.release_read();
...
TEST_CASE("DataBuffer: acquire_write returns the same slot every time") {
  DataBuffer<RGB> b;
  REQUIRE(b.allocate(1));
  RGB* a = b.acquire_write(); b.publish();
  RGB* c = b.acquire_write(); b.publish();
  CHECK(a == c);  // single slot — always the same pointer
}
...
TEST_CASE("DataBuffer: revision increments on each publish") {
  ...
  b.acquire_write()[0] = {1, 2, 3};
  b.publish();
  const uint32_t rev1 = b.revision();
  CHECK(rev1 != rev0);
  b.publish();
  CHECK(b.revision() != rev1);  // monotonically increasing
}

TEST_CASE("DataBuffer: try_acquire_read returns nullptr after release_read with no new publish") {
  DataBuffer<RGB> b;
  REQUIRE(b.allocate(4));
  b.acquire_write()[0] = {7, 7, 7};
  b.publish();
  REQUIRE(b.try_acquire_read() != nullptr);
  b.release_read();
  CHECK(b.try_acquire_read() == nullptr);  // nothing new
}
...
TEST_CASE("DataBuffer: SPSC cross-core consumer never tears under paced producer") {
  ...
  // per publish). Verifies release/acquire pairing: all bytes the producer
  // wrote before publish() are visible to the consumer after try_acquire_read().
  ...
  std::thread consumer([&] {
    while (!stop.load(std::memory_order_relaxed)) {
      const RGB* slot = b.try_acquire_read();
