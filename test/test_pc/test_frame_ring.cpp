// Cross-core SPSC tests for pmm::FrameRing.
...
#include "modules/lights/FrameRing.h"
#include "modules/lights/RGB.h"

using namespace pmm;

TEST_CASE("FrameRing: allocate(0) is a no-op that leaves the ring invalid") {
  FrameRing r;
  CHECK(r.allocate(0));      // zero-byte allocate succeeds
  CHECK(!r.valid());          // but no slots are owned
  CHECK(r.try_acquire_read() == nullptr);
}

TEST_CASE("FrameRing: allocate then read returns nullptr before first publish") {
  FrameRing r;
  REQUIRE(r.allocate(64 * sizeof(RGB)));
  REQUIRE(r.valid());
  CHECK(r.try_acquire_read() == nullptr);  // producer hasn't published yet
}

TEST_CASE("FrameRing: publish then read returns the just-filled slot") {
  FrameRing r;
  REQUIRE(r.allocate(4 * sizeof(RGB)));
...
TEST_CASE("FrameRing: slot rotation — successive publishes alternate slots") {
  FrameRing r;
  REQUIRE(r.allocate(sizeof(RGB)));
...
TEST_CASE("FrameRing: SPSC cross-core consumer never tears under paced producer") {
  ...
  // and trigger the "best-effort overwrite → tear" path documented in the
  // FrameRing.h header. Verifies the load-bearing invariant: when the
  // consumer keeps up, the release/acquire pairing delivers consistent
  // frames byte-by-byte.
  constexpr uint32_t kCount = 64;
  constexpr int      kIters = 5'000;
  FrameRing r;
  REQUIRE(r.allocate(kCount * sizeof(RGB)));
