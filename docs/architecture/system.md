...
- Find C's input that matches P. An input matches if its **name equals P's type** (`P.type()`) **or P's category** (`P.category()`) **or** its name is the conventional wildcard **`source`** — a type-agnostic "reads from whatever produces data" input (`ArtnetOutModule`, `PreviewModule` use it). Precedence: **exact type, then category, then `source`** (ADR 0007). An effect with inputs `layer` and `layout` dropped on a `layer` module → the `layer` input is chosen; on a `layout` module → the `layout` input; on a `ring-layout` module (category `layout`) → still the `layout` input, via the category match. A `preview` dropped on a `ripples` → its `source` input is chosen (no input named `ripples` or matching its category, but `source` is the wildcard). A precise `name == type` match wins over a category match, which wins over a generic `source`, if C has more than one.
