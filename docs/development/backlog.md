...
- **ArtNet-in module + UDP receive.** `modules/lights/ArtnetInModule.h` listens on UDP 6454 and exposes received DMX as a `PixelSource` (so `PreviewModule` and any downstream effect consume it just like a local effect). Controls: `universe` (0..15), `width`/`height`/`depth`. Owns its own RGB buffer; received DMX bytes copied in chunks indexed by incoming universe number. Adds `receive` to `pal::Udp` (today's send-only API landed minimally per [Rule #1](../architecture/process.md#1-minimalism-the-main-driver); the second caller justifies the addition). Unlocks when a host running QLC+ is available for HIL.
